...
class Downupblock(nn.Module):
    ...
    def forward(self, x):
        x1 = self.encoder(x)
        x2 = self.down(x1)
        high = x1 - F.interpolate(x2, size=x.size()[-2:], mode='nearest')
...
class CRBlock(nn.Module):
    ...
    def __init__(self, in_channel, out_channel, depth, ratio=1):
        ...
        self.rep1 = nn.Sequential(
            *[invertedBlock(in_channel=in_channel, out_channel=in_channel, ratio=ratio) for i in range(depth)])
